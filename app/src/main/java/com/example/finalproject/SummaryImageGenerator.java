...
public class SummaryImageGenerator {
        ...
        public static Bitmap generateHistorySummaryImage(
                ...
                String periodText = context.getString(R.string.summary_title);
                ...
                String dateText = context.getString(R.string.summary_month_prefix) + " " + month + " " + year;
                canvas.drawText(dateText, PADDING, currentY, subPeriodPaint);
                currentY += 80;

                // Draw summary cards
                drawSummaryCard(context, canvas, context.getString(R.string.summary_income), totalIncome, Color.rgb(40, 167, 69), currentY);
                currentY += 180;
                drawSummaryCard(context, canvas, context.getString(R.string.summary_expense), totalExpense, Color.rgb(220, 53, 69), currentY);
                currentY += 180;
                drawSummaryCard(context, canvas, context.getString(R.string.summary_balance), totalIncome - totalExpense,
                ...
                canvas.drawText(context.getString(R.string.summary_chart_title), PADDING, currentY, chartTitlePaint);
                currentY += 60;
                ...
                String dateStr = new SimpleDateFormat("dd/MM/yyyy HH:mm", Locale.getDefault())
                                .format(new Date());
                canvas.drawText(context.getString(R.string.summary_created) + " " + dateStr,
                                PADDING, IMAGE_HEIGHT - PADDING - 10, infoPaint);

                // Draw app signature on right
                String signature = context.getString(R.string.summary_generated);
        ...
        private static void drawSummaryCard(Context context, Canvas canvas, String title, double amount, int color, float y) {
        ...
        public static Bitmap generateCategorySummaryImage(
                ...
                titlePaint.setTypeface(Typeface.create(Typeface.DEFAULT, Typeface.BOLD));
                canvas.drawText(context.getString(R.string.category_summary_title), PADDING, currentY, titlePaint);
                currentY += 60;
                ...
                canvas.drawText(context.getString(R.string.summary_month_prefix) + " " + month + " " + year, PADDING, currentY, periodPaint);
                currentY += 80;
                ...
                canvas.drawText(context.getString(R.string.summary_total_expense), PADDING + 40, currentY + 60, totalTitlePaint);
                ...
                if (chart != null) {
                        ...
                        RectF chartCard = new RectF(PADDING, currentY, IMAGE_WIDTH - PADDING,
                ...
                if (categoryExpenses != null && !categoryExpenses.isEmpty()) {
                        for (Map.Entry<String, Double> category : categoryExpenses.entrySet()) {
                                ...
                                String localizedCategoryName = context.getString(
                                        CategoryConstants.getDisplayStringResource(category.getKey())
                                );
                                drawCategoryCard(canvas, localizedCategoryName, category.getValue(), percentage, currentY);
                                currentY += 100;
                        }
                }
                ...
                canvas.drawText(context.getString(R.string.summary_created) + " " + dateStr, PADDING, IMAGE_HEIGHT - PADDING - 10, infoPaint);

                String signature = context.getString(R.string.summary_generated);
                float signatureWidth = infoPaint.measureText(signature);
                canvas.drawText(signature, IMAGE_WIDTH - PADDING - signatureWidth, IMAGE_HEIGHT - PADDING - 10,
                                infoPaint);

                return bitmap;
        }

        private static void drawCategoryCard(Canvas canvas, String category, double amount, double percentage,
                        float y) {
                Paint cardPaint = new Paint();
                cardPaint.setColor(Color.WHITE);
